...
exports.webServerPluginForConfig = exports.webServer = exports.WebServerPlugin = void 0;
...
const webServerPluginForConfig = (config, reporter) => {
  ...
  const url = webServer.url || `http://localhost:${webServer.port}`; // We only set base url when only the port is given. That's a legacy mode we have regrets about.

  if (!webServer.url) process.env.PLAYWRIGHT_TEST_BASE_URL = url; // TODO: replace with reporter once plugins are removed.
  // eslint-disable-next-line no-console

  return new WebServerPlugin({ ...webServer,
    url
  }, webServer.port !== undefined, reporter);
};

exports.webServerPluginForConfig = webServerPluginForConfig;