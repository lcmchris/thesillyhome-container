...
class Runner {
  ...
  async _runFiles(list, filesByProject, testFileReFilters) {
    ...
    if (duplicateTitlesError) fatalErrors.push(duplicateTitlesError); // 6. Generate projects.
  ...
  async _performGlobalSetup(config, rootSuite) {
    const result = {
      status: 'passed'
    };
    ...
    await this._runAndReportError(async () => {
      ...
      if (config.webServer) this._plugins.push((0, _webServerPlugin.webServerPluginForConfig)(config, this._reporter)); // First run the plugins, if plugin is a web server we want it to run before the
      // config's global setup.

      for (const plugin of this._plugins) {
        var _plugin$setup;

        await ((_plugin$setup = plugin.setup) === null || _plugin$setup === void 0 ? void 0 : _plugin$setup.call(plugin, config, config._configDir, rootSuite));
      } // The do global setup.
