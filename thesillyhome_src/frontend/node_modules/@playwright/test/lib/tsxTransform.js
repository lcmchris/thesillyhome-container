...
function collectComponentUsages(node) {
  ...
  (0, _babelBundle.traverse)(node, {
    enter: p => {
      ...
      if (t.isJSXElement(p.node) && t.isJSXIdentifier(p.node.openingElement.name)) names.add(p.node.openingElement.name.name); // Treat mount(identifier, ...) as component usage.
...
function componentInfo(specifier, importSource, filename) {
  ...
  const importPath = isModuleOrAlias ? importSource : _path.default.resolve(_path.default.dirname(filename), importSource);
